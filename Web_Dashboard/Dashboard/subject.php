...
if (isset($_POST['upload-subject']))
{
$subject_n = $_POST['subject_name'];

include('../db_connection.php');
$sql="INSERT INTO subjects (sub_name ) VALUES ('$subject_n')";
...
{
  echo $Sub_id = $_POST['idd'];
  echo $Sub_name = $_POST['subject_name'];
  include('../db_connection.php');
  $qry = "UPDATE subjects SET sub_name = '$Sub_name' WHERE id = '$Sub_id'";
  $result = mysqli_query($db_con, $qry);
}
    ...
    <div id="popupmenu"> 
      <div class="popup-header">
        <div class="title">Update Subject</div>
          <a  href="subject.php"><button data-close-button class="close-button" >&times;</button></a>
        </div>
      <div class="popup-body">
        <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
		    Subject Name:	 <input type="text" class="input-bar" name="subject_name" value="<?php echo $S_name?>" style="height:30px; border:solid 1px grey; margin-left: 2px;"/></br><br>
        <input type="hidden" name="idd" value=" <?php echo $S_id ?>"/>
        <input type="submit" name="subject" value="Update" class="form-button" style="height: 30px; width: 70px; border:none; border-radius: 4px; background-color: green; color: white; margin-bottom: 10px;">
        </form>
      </div>     
    </div>
  <div id="overlay"></div> 
...
{ ?>
  ...
  <div class="app-content">
    ...
    <button class="app-content-headerButton" data-modal-target="#modal" > <label for="attendance-marking" class>Add Subject</label></button>    
</div>
    ...
    <div class="products-area-wrapper tableView">
      ...
      <?PHP
            ...
            {            
            $search = $_POST['search'];
            $sav = "SELECT * FROM subjects WHERE sub_name like '%{$search}%'";
            }
            else{
              $sav = "SELECT * FROM subjects";
            }
            ...
            ?>
              <form action="<?php $_SERVER['PHP_SELF'] ?>" method="POST">  
                <div class="products-row">
                  ...
                  <div class="product-cell Stock"><span class="cell-label"  >Subject:</span><?php echo $row['sub_name']; ?></div>        
                  <input type="hidden" name="id" value=" <?php echo $row['id']; ?>"/>
                  <input type="hidden" name="subject_name" value=" <?php echo $row['sub_name']; ?>"/>
                  <div class="product-cell Stock"> <span class="cell-label"  >Action:</span><input type="submit" name="delete-subject" value="Delete" class="Delete-btn" > 
                  <span class="cell-label" >Action:</span><input type="submit" value="Update" class="update-btn" name="update-btn"/>
                  </div>
          </div>
	...
	<center><form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
		Subject Name:	 <input type="text" class="input-bar" name="subject_name" style="height:30px; border:solid 1px grey; margin-left: 2px;"/></br><br>
	<input type="submit" name="upload-subject" value="Upload" class="form-button" style="height: 30px; width: 70px; border:none; border-radius: 4px; background-color: green; color: white; margin-bottom: 10px;">
	</form></center>
</div>
 </div>
  </div>
